...
Defines data types and enums to be used with pixis. See PICam user manual for more information.
...
# +++++++ simple data types +++++++++++++++++++++++++++++++++++++++++++++++++++
# data types - use these for communication with the pixis library!
...
# +++++++ enumerations +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# each enumeration has a lookup table of the same name XXLookup which accepts
# the integer value as key and gives the string name as value
PicamError = {
    "None": 0,
    "UnexpectedError": 4,
...
PicamEnumeratedTypeLookup = dict(zip(PicamEnumeratedType.values(),
                                     PicamEnumeratedType.keys()))
...
PicamComputerInterfaceLookup = dict(zip(PicamComputerInterface.values(),
                                        PicamComputerInterface.keys()))
...
PicamStringSizeLookup = dict(zip(PicamStringSize.values(),
                                 PicamStringSize.keys()))
...
PicamConstraintTypeLookup = dict(zip(PicamConstraintType.values(),
                                     PicamConstraintType.keys()))

PI_V = lambda v, c, n:\
    (PicamConstraintType[c] << 24) + (PicamValueType[v] << 16) + n
...
PicamAdcAnalogGainLookup = dict(zip(PicamAdcAnalogGain.values(),
                                    PicamAdcAnalogGain.keys()))
...
PicamAdcQualityLookup = dict(zip(PicamAdcQuality.values(),
                                 PicamAdcQuality.keys()))
...
PicamGatingModeLookup = dict(zip(PicamGatingMode.values(),
                                 PicamGatingMode.keys()))

PicamGatingSpeed = {
    "Fast": 1,
    "Slow": 2
}
PicamGatingSpeedLookup = dict(zip(PicamGatingSpeed.values(),
                                  PicamGatingSpeed.keys()))
...
PicamIntensifierOptionsMaskLookup = dict(zip(
    PicamIntensifierOptionsMask.values(), PicamIntensifierOptionsMask.keys()))
...
PicamModulationTrackingMaskLookup = dict(zip(
    PicamModulationTrackingMask.values(), PicamModulationTrackingMask.keys()))
...
PicamOrientationMaskLookup = dict(zip(PicamOrientationMask.values(),
                                      PicamOrientationMask.keys()))
...
PicamOutputSignalLookup = dict(zip(PicamOutputSignal.values(),
                                   PicamOutputSignal.keys()))

PicamPhosphorType = {
    "P43": 1,
    "P46": 2
}
PicamPhosphorTypeLookup = dict(zip(PicamPhosphorType.values(),
                                   PicamPhosphorType.keys()))
...
PicamPhotocathodeSensitivityLookup = dict(zip(
    PicamPhotocathodeSensitivity.values(), PicamPhotocathodeSensitivity.keys()))
...
PicamPixelFormatLookup = dict(zip(PicamPixelFormat.values(),
                                  PicamPixelFormat.keys()))
...
PicamSensorTemperatureStatusLookup = dict(zip(
    PicamSensorTemperatureStatus.values(), PicamSensorTemperatureStatus.keys()))

PicamSensorType = {
    "Ccd": 1,
    "InGaAs": 2
}
PicamSensorTypeLookup = dict(zip(PicamSensorType.values(),
                                 PicamSensorType.keys()))
...
PicamTimeStampsMaskLookup = dict(zip(PicamTimeStampsMask.values(),
                                     PicamTimeStampsMask.keys()))

PicamTriggerCoupling = {
    "AC": 1,
    "DC": 2
}
PicamTriggerCouplingLookup = dict(zip(PicamTriggerCoupling.values(),
                                      PicamTriggerCoupling.keys()))
...
PicamTriggerResponseLookup = dict(zip(PicamTriggerResponse.values(),
                                      PicamTriggerResponse.keys()))

PicamTriggerSource = {
    "External": 1,
    "Internal": 2
}
PicamTriggerSourceLookup = dict(zip(PicamTriggerSource.values(),
                                    PicamTriggerSource.keys()))
...
PicamValueAccessLookup = dict(zip(PicamValueAccess.values(),
                                  PicamValueAccess.keys()))

PicamConstraintScope = {
    "Independent": 1,
    "Dependent": 2
}
PicamConstraintScopeLookup = dict(zip(PicamConstraintScope.values(),
                                      PicamConstraintScope.keys()))
...
PicamRoisConstraintRulesMaskLookup = dict(zip(
    PicamRoisConstraintRulesMask.values(), PicamRoisConstraintRulesMask.keys()))
