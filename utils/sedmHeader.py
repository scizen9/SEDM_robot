...
def ra_to_deg(ra):
    hr, mn, sec = map(float, ra.split(":"))

    return 15 * hr + 0.25 * mn + 0.0042 * sec


def dec_to_deg(dec):
    sign = 1
    deg, mn, sec = map(float, dec.split(":"))
    if deg <= 0:
        sign = -1
    return (abs(deg) + mn / 60. + sec / 3600.) * sign


def set_object_coord(coord=-999, coord_type='ra'):
    # print(coord, 'Coord', coord_type)
...
class addHeader():
    ...
    def set_header(self, image, obsdict):
        ...
        prihdr.set("LST", obsdict["lst"],
                   "Local Sideral Time at Start of Observation")
        prihdr.set("MJD_OBS",  float(obsdict['julian_date']) - 2400000.5,
                   "Local Sideral Time at Start of Observation")
        ...
        prihdr.set("DEC", obsdict["telescope_dec"],
                   "Telscope Dec position start ")
        prihdr.set("TEL_DEC", obsdict["telescope_dec"],
                   "Telscope Dec position start ")
        prihdr.set("TEL_AZ", obsdict["telescope_azimuth"],
                   "Telescope Azimuth(degrees)")
        prihdr.set("TEL_EL", obsdict["telescope_elevation"],
                   "Telescope Elevation")
        prihdr.set("AIRMASS", obsdict["telescope_airmass"], "Telescope Airmass")
        prihdr.set("TEL_PA", obsdict["telescope_parallactic"],
                   "Telescope Parallactic Angle")
        prihdr.set("RA_RATE", obsdict["telescope_ra_rate"], "Telescope Ra Rate")
        prihdr.set("DEC_RATE", obsdict["telescope_dec_rate"],
                   "Telescpoe Dec Rate")
        prihdr.set("RA_OFF", obsdict["telescope_ra_offset"],
                   "Telescope RA Offset")
        prihdr.set("DEC_OFF", obsdict["telescope_dec_offset"],
                   "Telescope Dec Offset")
        prihdr.set("TELHASP", obsdict["telescope_ha_speed"],
                   "Telescope HA Speed")
        prihdr.set("TELDECSP", obsdict["telescope_dec_speed"],
                   "Telescope Dec Speed")
        prihdr.set("RA_REFR", obsdict["telescope_ha_refr(arcsec)"],
                   "Telescope RA Refraction")
        prihdr.set("DEC_REFR", obsdict["telescope_dec_refr(arcsec)"],
                   "Telescope Dec Refraction")
        prihdr.set("FOCPOS", obsdict["focus_position"],
                   "Position of secondary focus")
        prihdr.set("IFUFOCUS", obsdict["ifufocus"],
                   "Position of IFU Focus Stage")
        prihdr.set("IFUFOC2", obsdict["ifufoc2"],
                   "Position of RC + IFU Parfocal Stage")
        prihdr.set("DOMEST", obsdict["dome_shutter_status"],
                   "Dome Shutter Status")
        ...
        prihdr.set("WSCRMO", obsdict["ws_motion_mode"],
                   "Windscreen motion mode")
        prihdr.set("TELCONT", obsdict["telescope_control_status"],
        ...
        prihdr.set("REMOTST", obsdict["remote_close_status"],
                   "Remote Close Status")
        prihdr.set("TELRDST", obsdict["telescope_ready_status"],
                   "Telescope Ready Status")
        prihdr.set("HAAX_ST", obsdict["ha_axis_hard_limit_status"],
                   "HA Axis Hard Limit Status")
        prihdr.set("FOCSTAT", obsdict["focus_hard_limit_status"],
                   "Focus Hard Limit Status")
        prihdr.set("DEC_AX", obsdict["dec_axis_hard_limit_status"],
                   "Dec Axis Hard Limit Status")
        ...
        prihdr.set("OBJEQX", obsdict["object_equinox"],
                   "Object Coordinate Equinox")
        ...
        prihdr.set("TEL_MO", obsdict["telescope_motion_status"],
                   "Telescope Motion Status")
        prihdr.set("WSCR_EL", obsdict["windscreen_elevation"],
                   "Windscreen Elevation")
        prihdr.set("SOL_RA", obsdict["solar_ra"], "Solar RA")
        prihdr.set("SOL_DEC", obsdict["solar_dec"], "Solar Dec")
        prihdr.set("WIND_DIR", obsdict["wind_dir_current"],
                   "Wind direction (degrees)")
        prihdr.set("WSP_CUR", obsdict["windspeed_current"],
                   "Current windspeed (mph)")
        prihdr.set("WSP_AVG", obsdict["windspeed_average"],
        ...
        prihdr.set("PRI_TEMP", obsdict["primary_cell_temp"],
                   "Primary Cell Temp")
        prihdr.set("SEC_TEMP", obsdict["secondary_cell_temp"],
                   "Secondary Cell Temp")
        ...
        prihdr.set("FILTER", obsdict["filter"],
                   "Filter Where Object is Located ")
        prihdr.set("ABPAIR", obsdict["abpair"],
                   "Is observation part of AB Pair")
        ...
        try:
            # prihdr.set("CRVAL1", round(ra_to_deg('05:23:33') - 0.03333, 5),
            # "Center RA value")
            prihdr.set("CRVAL1", ra_to_deg(obsdict["telescope_ra"]) - 0.03333,
                       "Center RA value")
        except:
            prihdr.set("CRVAL1", -999, "Failed to calculate")
            pass
        try:
            # prihdr.set("CRVAL2", round(dec_to_deg('33:23:33') - 0.03333, 5),
            # "Center Dec value")
        ...
        prihdr.set("SECPRESS", obsdict["sec_vacuum"],
                   "Secondary Vacuum Pressure")
        ...
        prihdr.set("END_DEC", obsdict["end_dec"],
                   "Telescope DEC at shutter close")
        ...
        prihdr.set("ENDSECPR", obsdict["endsecpr"],
                   "End Secondary Vacuum Pressure")
        prihdr.set("ELAPTIME",  round(time.time() - obsdict['starttime'], 3),
                   "Elapsed time of observation")
        hdulist.close()
...
if __name__ == "__main__":
    x = addHeader()
    y = {'utc': '2019:245:12:13:02.2', 'telescope_id': '60',
         'telescope_control_status': 'remote', 'lamp_status': 'off',
         'lamp_current': '0.00', 'dome_shutter_status': 'closed',
         'ws_motion_mode': 'bottom', 'dome_motion_mode': 'anticipate',
         'telescope_power_status': 'ready', 'oil_pad_status': 'ready',
         'weather_status': 'ready', 'sunlight_status': 'okay',
         'remote_close_status': 'not_okay', 'telescope_ready_status': 'ready',
         ...
         'focus_soft_limit_status': 'okay', 'focus_motion_status': 'stationary',
         'east_soft_limit_value': '-6.4', 'west_soft_limit_value': '6.4',
         'north_soft_limit_value': '109.5', 'south_soft_limit_value': '-41.8',
         'horizon_soft_limit_value': '10.0',
         'ha_axis_soft_limit_status': 'okay',
         'dec_axis_soft_limit_status': 'okay',
         'horizon_soft_limit_status': 'okay', 'windspeed_avg_threshold': '25.0',
         'gust_speed_threshold': '35.0', 'gust_hold_time': '900',
         'outside_dewpt_threshold': '2.0', 'inside_dewpt_threshold': '2.0',
         'wetness_threshold': '500', 'wind_dir_current': '63',
         'windspeed_current': '3.7', 'windspeed_average': '5.7',
         'outside_air_temp': '20.5', 'outside_rel_hum': '65.7',
         'outside_dewpt': '13.9', 'inside_air_temp': '21.3',
         'inside_rel_hum': '55.9', 'inside_dewpt': '12.1',
         'mirror_temp': '21.2', 'floor_temp': '21.7', 'bot_tube_temp': '21.0',
         'mid_tube_temp': '21.3', 'top_tube_temp': '21.4',
         'top_air_temp': '21.3', 'primary_cell_temp': '21.2',
         'secondary_cell_temp': '21.2', 'wetness': '-271', 'lst': '03:11:01.3',
         'julian_date': '2458729.0090534', 'apparent_equinox': '2019.67',
         'telescope_equinox': 'j2000.0', 'telescope_ha': 'e00:51:28.85',
         'telescope_ra': '04:01:38.90', 'telescope_dec': '-20:31:56.3',
         'telescope_ra_rate': '0.00', 'telescope_dec_rate': '0.00',
         'telescope_ra_offset': '13166.08', 'telescope_dec_offset': '-110.87',
         'telescope_azimuth': '165.34', 'telescope_elevation': '34.84',
         'telescope_parallactic': '347', 'telescope_ha_speed': '-0.0046',
         'telescope_dec_speed': '0.0000', 'telescope_ha_refr(arcsec)': '-15.25',
         'telescope_dec_refr(arcsec)': '-65.88',
         'telescope_motion_status': 'stopped', 'telescope_airmass': '1.747',
         'telescope_ref_ut': '11.840406', 'object_name': '"simulated"',
         'object_equinox': 'j2000.0', 'object_ra': '03:46:01.55',
         'object_dec': '-20:30:27.9', 'object_ra_rate': '0.00',
         'object_dec_rate': '0.00', 'object_ra_proper_motion': '0.000000',
         'object_dec_proper_motion': '0.00000', 'focus_position': '15.81',
         'dome_gap(inch)': '-240', 'dome_azimuth': '233.3',
