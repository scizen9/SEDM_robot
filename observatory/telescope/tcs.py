...
class Telescope:
    ...
    def __init__(self, simulated=False, gxnaddress=None):
        ...
        with open(os.path.join(SITE_ROOT, 'config', 'tcs.json')) as cfile:
            self.tcs_config = json.load(cfile)

        if not gxnaddress:
            self.address = (self.tcs_config['gxn_address'],
                            self.tcs_config['gxn_port'])
        else:
        ...
        self.offset_direction_list = ['N', 'E', 'W', 'S']
        self.offset_direction_time_list = ['ES', 'WS']
        self.takecontrol()
    ...
    def check_return(self, int_return):
        ...
        if int_return == 0:
            ...
            self.error_str = "-6: GXN interface does not have control"
        else:
    ...
    def offset_by_time(self, direction="", duration=""):
        """
        ES, WS (SLOW) offsets the telescope position east or west respectively
        by the requested number of seconds of time, moving at the rate set by
        MRATES. ?POS item Telescope_Motion_Status must be TRACKING or
        IN_POSITION for this command to work, and will change to MOVING during
        the move.  The command fails if wetness is sensed.
    ...
    def offset(self, ra=0, dec=0):
        ...
        try:
            offsets = [round(float(xc), 2) for xc in [ra, dec]]
    ...
    def pt_offset_time(self, ra=0, dec=0):
        ...
        try:
            offsets = [round(float(xc), 3) for xc in [ra, dec]]
            if abs(offsets[0]) >= 600:
    ...
    def rates(self, ra=0, dec=0):
        ...
        try:
            rates = [round(float(xc), 2) for xc in [ra, dec]]
            if abs(rates[0]) >= 100000:
                return False, '%ss is larger than 100000"/hr RA limit' % \
                       rates[0]
            if abs(rates[1]) >= 100000:
                return False, '%ss is larger than 100000"/hr DEC limit' % \
                       rates[1]
        except Exception as e:
            return False, str(e)
    ...
    def stow(self, ha=0, dec=109, domeaz=90):
        ...
        """
        try:
            stow = [round(float(xc), 3) for xc in [ha, dec, domeaz]]
            if abs(stow[0]) >= 24:
    ...
    def coords(self, name=None, ra=0, dec=33, equinox=2000, ra_rate=0,
               dec_rate=0, motion_flag=0, epoch=""):
        """
        ...
        try:
            ...
            coords = [round(float(xc), 5) for xc in [ra, dec, equinox,
                                                     ra_rate, dec_rate]]
